:samples-dir: /home/runner/work/gradle/gradle/platforms/documentation/docs/build/working/samples/install/building-groovy-applications-multi-project
:gradle-version: 9.0.0-rc-2

= Building Groovy Applications with libraries Sample

[.download]
- link:zips/sample_building_groovy_applications_multi_project-groovy-dsl.zip[icon:download[] Groovy DSL]
- link:zips/sample_building_groovy_applications_multi_project-kotlin-dsl.zip[icon:download[] Kotlin DSL]
...
In this guide, you'll learn how to structure such a project on the example of a Groovy application.
...
You'll build a Groovy application that consists of an _application_ and multiple _library_ projects.
...
<8> Groovy source folders in each of the subprojects
<9> Groovy test source folders in the subprojects

You now have the project setup to build a Groovy application which is modularized into multiple subprojects.
...
====
include::sample[dir="kotlin",files="buildSrc/src/main/kotlin/buildlogic.groovy-common-conventions.gradle.kts[]"]
include::sample[dir="groovy",files="buildSrc/src/main/groovy/buildlogic.groovy-common-conventions.gradle[]"]
====

The `groovy-common-conventions` defines some configuration that should be shared by all our Groovy project -- independent of whether they represent a library or the actual application.
First, we apply the link:{userManualPath}/groovy_plugin.html[Groovy Plugin] *(1)* to have all functionality for building Groovy projects available.
...
====
include::sample[dir="kotlin",files="buildSrc/src/main/kotlin/buildlogic.groovy-library-conventions.gradle.kts[]"]
include::sample[dir="groovy",files="buildSrc/src/main/groovy/buildlogic.groovy-library-conventions.gradle[]"]
====
====
include::sample[dir="kotlin",files="buildSrc/src/main/kotlin/buildlogic.groovy-application-conventions.gradle.kts[]"]
include::sample[dir="groovy",files="buildSrc/src/main/groovy/buildlogic.groovy-application-conventions.gradle[]"]
====


Both `groovy-library-conventions` and `groovy-application-conventions` apply the `groovy-common-conventions` plugin *(1)* so that the configuration performed there is shared by library and application projects alike.
...
* Second, if the project has dependencies, a `dependencies {}` block should be added.
  Dependencies can be external, such as the JUnit dependencies we add in `groovy-common-conventions`, or can point to other local subprojects.
...
Apart from the Gradle build files, you can find example Groovy source code and test source code in the corresponding folders.
