:samples-dir: /home/runner/work/gradle/gradle/platforms/documentation/docs/build/working/samples/install/building-scala-applications-multi-project
:gradle-version: 9.0.0-rc-1

= Building Scala Applications with libraries Sample

[.download]
- link:zips/sample_building_scala_applications_multi_project-groovy-dsl.zip[icon:download[] Groovy DSL]
- link:zips/sample_building_scala_applications_multi_project-kotlin-dsl.zip[icon:download[] Kotlin DSL]
...
In this guide, you'll learn how to structure such a project on the example of a Scala application.
...
You'll build a Scala application that consists of an _application_ and multiple _library_ projects.
...
When prompted, select the `1: application` project type and `4: Scala` as the implementation language.
...
[listing.terminal.sample-command,user-inputs="1|4|1|||"]
----
$ gradle init
...
<8> Scala source folders in each of the subprojects
<9> Scala test source folders in the subprojects

You now have the project setup to build a Scala application which is modularized into multiple subprojects.
...
====
include::sample[dir="kotlin",files="buildSrc/src/main/kotlin/buildlogic.scala-common-conventions.gradle.kts[]"]
include::sample[dir="groovy",files="buildSrc/src/main/groovy/buildlogic.scala-common-conventions.gradle[]"]
====

The `scala-common-conventions` defines some configuration that should be shared by all our Scala project -- independent of whether they represent a library or the actual application.
First, we apply the link:{userManualPath}/scala_plugin.html[Scala Plugin] *(1)* to have all functionality for building Scala projects available.
...
====
include::sample[dir="kotlin",files="buildSrc/src/main/kotlin/buildlogic.scala-library-conventions.gradle.kts[]"]
include::sample[dir="groovy",files="buildSrc/src/main/groovy/buildlogic.scala-library-conventions.gradle[]"]
====
====
include::sample[dir="kotlin",files="buildSrc/src/main/kotlin/buildlogic.scala-application-conventions.gradle.kts[]"]
include::sample[dir="groovy",files="buildSrc/src/main/groovy/buildlogic.scala-application-conventions.gradle[]"]
====


Both `scala-library-conventions` and `scala-application-conventions` apply the `scala-common-conventions` plugin *(1)* so that the configuration performed there is shared by library and application projects alike.
...
* Second, if the project has dependencies, a `dependencies {}` block should be added.
  Dependencies can be external, such as the JUnit dependencies we add in `scala-common-conventions`, or can point to other local subprojects.
...
Apart from the Gradle build files, you can find example Scala source code and test source code in the corresponding folders.
