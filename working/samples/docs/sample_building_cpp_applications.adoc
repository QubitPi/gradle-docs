:samples-dir: /home/runner/work/gradle/gradle/platforms/documentation/docs/build/working/samples/install/building-cpp-applications
:gradle-version: 9.0.0-rc-2

= Building {cpp} Applications Sample

[.download]
- link:zips/sample_building_cpp_applications-groovy-dsl.zip[icon:download[] Groovy DSL]
- link:zips/sample_building_cpp_applications-kotlin-dsl.zip[icon:download[] Kotlin DSL]

NOTE: You can open this sample in an link:{userManualPath}/gradle_ides.html#gradle_ides[IDE that supports Gradle].

This guide demonstrates how to create a {cpp} application with Gradle using `gradle init`.
...
You'll generate a {cpp} application that follows Gradle's conventions.
...
When prompted, select the `1: application` project type and `5: {cpp}` as the implementation language.
Next you can choose the DSL for writing buildscripts -  `1  : Kotlin` or `2: Groovy`.
For the other questions, press enter to use the default values.

The output will look like this:

[listing.terminal.sample-command,user-inputs="1|5|1|||"]
----
$ gradle init
...
You now have the project setup to build a {cpp} application.
...
Our build contains one subproject called `app` that represents the {cpp} application we are building.
...
[source,cpp]
----
include::{samples-dir}/groovy/app/src/main/cpp/app.cpp[]
----
...
[source,cpp]
----
include::{samples-dir}/groovy/app/src/test/cpp/app_test.cpp[]
----
...
That's it! You've now successfully configured and built a {cpp} application project with Gradle.
You've learned how to:

* Initialize a project that produces a {cpp} application
* Build, bundle and run the application

== Next steps

To learn more about how you can further customize {cpp} application projects, check out the user manual chapter on
link:{userManualPath}/building_cpp_projects.html[Building {cpp} projects].
